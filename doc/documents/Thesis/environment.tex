...
This means that floating-point calculations have to be emulated with other operations.
...
On AVRs a double is as large as a float and has 4 bytes. In result of this fact the usual precision are 6-7 digits. % <--- Fragen
...
For an illustration of PWM see figure~\ref{fig:pwm}. %<-- Fragen
...
To output a PWM signal the port has to be set as output. A general description can be found in the section~\ref{sec:digitalOutputPins}. %<-- Fragen
...
UART is a serial interface and it is one of the most important interfaces on PCs and microcontrollers.
%<--Fragen
%Today in regular new PC the RS-232 interfaces are out of date because the USB have suppressed the RS-232.
RS-232 interfaces are nowadays replaced by USB
...
%On the Arduino~Mega~2560 the USB to serial interface is programmed on an ATmega16u2.
To use the RS-232 interface on newer PCs it is possible to tunnel the RS-232 trough USB or Bluetooth. 
The OS of the PC creates a virtual RS-232 interface.
To create a tunnel of a RS-232 interface above USB it is possible to use an IC which have such a function.
A example for such a chip is the CP2102.
Another possibility is to program a microcontroller to a RS-232 tunnel.
This is also the method which is used on the Arduino.
As an example on the Arduino~Mega~2560 the serial interface is created by an ATmega16u2.
