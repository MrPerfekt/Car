...
LCDShield::LCDShield()
{
	...
	DDRB = ((1<<DIO)|(1<<SCK_LCD));     //Set DIO and SCK_LCD pins on PORTB as outputs
	DDRH = ((1<<CS)|(1<<LCD_RES));      //Set CS and RES pins PORTH as outputs
#else
	DDRB = ((1<<CS)|(1<<DIO)|(1<<SCK_LCD)|(1<<LCD_RES));  //Set the control pins as outputs
#endif
...
void LCDShield::LCDCommand(unsigned char data)
{
	...
	cbi(LCD_PORT_SCK, SCK_LCD);   // send clock pulse
	delayMicroseconds(1);
	sbi(LCD_PORT_SCK, SCK_LCD);
...
void LCDShield::LCDData(unsigned char data)
{
	...
	cbi(LCD_PORT_SCK, SCK_LCD);   // send clock pulse
	delayMicroseconds(1);
	sbi(LCD_PORT_SCK, SCK_LCD);   // send clock pulse
...
void LCDShield::init(int type)
{
	...
	cbi(LCD_PORT_SCK, SCK_LCD);     //CLK = LOW
	...
	delay(200);
	sbi(LCD_PORT_SCK, SCK_LCD);     // SCK_LCD = HIGH
	...
	//LCDCommand(INVOFF);   // disinvert display mode(PHILIPS) 
	//LCDCommand(INVON);   // invert display mode(PHILIPS) 
	//LCDCommand(NORON);   // display mode(PHILIPS) 
