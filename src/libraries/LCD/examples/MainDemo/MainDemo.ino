	...
	LCDInit();	    //Initialize the LCD
        ...
        while(a == 1)
        {
          if(b<100){
            ...
            LCDPutStr("Pete is uggly!", 0, 4, GREEN, WHITE); // Write instructions on display
            LCDClear(WHITE);    // Clear LCD to a solid color
          }
          else{
            LCDPutStr("Pete gets all", 0, 4, BLUE, WHITE); // Write instructions on display
            LCDPutStr("the girls!", 16, 4, BLUE, WHITE);
            LCDClear(WHITE);    // Clear LCD to a solid color
          }
          if(b>200){
          {b=0;}
